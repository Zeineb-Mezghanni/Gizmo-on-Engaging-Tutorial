...
- An account on Engaging and the ability to `ssh` in our group allocation is on ```OS R8.10```. 
---

#### 1. Clone the code from [this repository](https://github.com/pfhopkins/gizmo-public) 
```
git clone https://github.com/pfhopkins/gizmo-public
cd gizmo-public
```

**!!!! More work to be done here: check with Abdelaziz about the code he fixed last year see [Appendix](#Appendix)**
...
- `Config.sh` controls **which physics modules** are enabled.
- `Makefile.systype` controls **which machine** you are building on.


#### 3. Set the system type

Open `Makefile.systype` and make sure this line is **uncommented**:

```makefile
SYSTYPE="engaging"
```
If it's not aleady there, add it, and k eep **all other** `SYSTYPE` lines commented out (lines start with `#`).


#### 4. Modify the following lines like this in the Makefile:

Open the main `Makefile` and find the `engaging` block. It should look like this:
...
**Note: paths can change when modules are updated.** Use "module show <module-name>" to check the paths, for example, "module show fftw/3.3.10"
...
#### 6. Check all libraries are available:
...
You now have a working `GIZMO` executable on Engaging. To run a simulation you'll submit a SLURM job that loads the same modules and calls `mpirun ./GIZMO <params.txt>`.
More on slurm files can be found in the engaging documentations and/or discussed with Zeineb later. 
